...
// ------------------
{
    ...
    cudaHostNodeParams hostParams_1 = {0};
    cudaHostNodeParams hostParams_2 = {0};

    send_args_t send_args_1 = {rank, ranks_count, Fnew_D, Fnew_D, local_args, comm};
    send_args_t send_args_2 = {rank, ranks_count, F_D, F_D, local_args, comm};

    hostParams_1.fn = (cudaHostFn_t)transfer_data_nccl;
    hostParams_2.fn = (cudaHostFn_t)transfer_data_nccl;

    hostParams_1.userData = &send_args_1;
    hostParams_2.userData = &send_args_2;

    // cudaKernelNodeParams kernel_params_1 = {0};
    // cudaKernelNodeParams kernel_params_2 = {0};

    // void* kernel_args_1[] = { F_D, Fnew_D, args_d };
    // void* kernel_args_2[] = { Fnew_D, F_D, args_d };

    // kernel_params_1.func = (void*)iterate;
    // kernel_params_1.gridDim = threadPerBlock;
    // kernel_params_1.blockDim = blocksPerGrid;
    // kernel_params_1.sharedMemBytes = 0;
    // kernel_params_1.kernelParams = (void**)kernel_args_1;

    // kernel_params_2.func = (void*)iterate;
    // kernel_params_2.gridDim = threadPerBlock;
    // kernel_params_2.blockDim = blocksPerGrid;
    // kernel_params_2.sharedMemBytes = 0;
    // kernel_params_2.kernelParams = (void**)kernel_args_2;

    
    for (size_t i = 0; i < ITERS_BETWEEN_UPDATE / 2; i++) {
            // cudaGraphNode_t kernel_node_1;
            // cudaGraphAddKernelNode(&kernel_node_1, graph, nullptr, 0, &kernel_params_1);
            // // iterate<<<blocksPerGrid, threadPerBlock, 0, stream>>>(F_D, Fnew_D, args_d);

            cudaGraphNode_t hostNode_1;
            cudaGraphAddHostNode(&hostNode_1, graph, NULL, 0, &hostParams_1);
            // transfer_data_nccl(rank, ranks_count, Fnew_D, Fnew_D, local_args, comm);

            // cudaGraphNode_t kernel_node_2;
            // cudaGraphAddKernelNode(&kernel_node_2, graph, nullptr, 0, &kernel_params_2);
            // // iterate<<<blocksPerGrid, threadPerBlock, 0, stream>>>(Fnew_D, F_D, args_d);

            cudaGraphNode_t hostNode_2;
            cudaGraphAddHostNode(&hostNode_2, graph, NULL, 0, &hostParams_2);
    ...
    do {
        ...
        // double* err_red;
        // cudaMallocHost(&err_red, sizeof(double) * num_blocks_reduce);
        // cudaMemcpy(err_red, error_reduction, sizeof(double) * num_blocks_reduce, cudaMemcpyDeviceToHost);
